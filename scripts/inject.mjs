...
for (const p of [
  'why did we drop polling for the order feed',
  'the websocket reconnect is looping again',
  'add pagination to the invoice table',
])
...
const store = await createStore(join(dir, 'i.db'));
await store.init();
...
const seed = [
  [
    ...
    'Polling hammered the API and still lagged.',
  ],
  [
    'deadend',
    'Redux middleware for socket events did not scale',
    'Re-renders across the whole tree.',
  ],
  ['bugfix', 'Fixed reconnect storm on token refresh', 'Auth refresh raced the retry timer.'],
  [
    'pattern',
    'Virtualized the invoice table with react-window',
    '5k rows blocked the main thread.',
  ],
];
const obs = [];
for (const [k, t, b] of seed) {
  const [e] = await emb.embed([`${t} ${b}`]);
  obs.push({
    id: randomUUID(),
    sessionId: 's',
    project,
...
const none = renderPromptContext(
  await search.search({ text: 'kubernetes helm chart rollout', project, limit: 4 }),
  500,
);
...
ck(
  'short ids resolve to full observations',
  full.length === hits.length,
  `${full.length}/${hits.length}`,
);
ck(
  'bodies came back',
