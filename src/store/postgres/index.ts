...
export class PostgresStore implements MemoryStore {
  ...
  async insertObservations(observations: Observation[]): Promise<void> {
    ...
    try {
      ...
      for (const obs of observations) {
        const columns =
          `id, session_id, project, kind, title, body, files, tags, created_at` +
          (this.vectorEnabled && obs.embedding ? ', embedding' : '');
        ...
        if (this.vectorEnabled && obs.embedding) {
          values.push(`[${obs.embedding.join(',')}]`);
        }
        const holes = values.map((_, i) => `$${i + 1}`).join(', ');
        await client.query(
          `INSERT INTO observations (${columns}) VALUES (${holes})
  ...
  async clear(project?: string): Promise<number> {
    const where = project ? 'WHERE project = $1' : '';
    ...
    try {
      ...
      await client.query(`DELETE FROM sessions ${where}`, values);
      await client.query('COMMIT');
