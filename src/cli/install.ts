...
const INSTRUCTIONS = [
  ...
  '',
  '`search` covers *why*: past decisions, bugs, reasoning. It does not know',
  '*where* something is used right now — for that, call `find_usages` instead',
  'of grepping yourself, specifically before editing or removing a shared or',
  'exported name, and when asked what uses something. It re-derives the answer',
  'from the current source on every call, so it is never stale. Plain-text or',
  'log searching is still a job for grep.',
  '',
